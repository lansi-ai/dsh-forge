...
- **解法**（两条链一起补，2026-09-16）：
  ...
  7. **让「预发布渠道」真正生效**（2026-09-16 追加）：把「渠道 → feed / 是否纳入预发布」抽成纯模块 `src/forge-host/updater-channels.ts`（`CHANNEL_FEED` + `allowPrereleaseFor`），`syncChannelFeed()` 显式写 `autoUpdater.allowPrerelease = allowPrereleaseFor(channel)` —— 覆盖上游「按当前版本推导」的默认值：**正式版装机选「预发布渠道」也能自动升到 `-rc.N`**，切回正式渠道即严格只看正式版；`test/updater-channel.test.cjs` 锁死该规则防回退。⚠ 只在**含它的构建**里生效：跑着的旧版本（如 0.1.1）仍需手动装一次预发布包。
- **复盘要点**：
