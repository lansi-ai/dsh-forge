...
| （无官方对应，**独立插件包** · 坑 74） | `dsh-llm-opencode-session`（**已发布**：`github.com/lansi-ai/dsh-llm-opencode-session` @ **`v0.1.1`**（tag）/ `a9778a4`（2026-09-16；`v0.1.0` = `e54cbbd` root commit）；工作区 `E:\Projects\DSH\plugins\dsh-llm-opencode-session\`，与 `dsh-llm-app-credentials` 同级） | **opencode 逐会话头插件**：`apply` 经 `ctx.effect` 包装 `globalThis.fetch`，只对 `opencode.ai` 域注入 `x-opencode-session`（= **人可读短标签** `dsh-<中文2字><字母2>`，如 `dsh-青竹aB`，由 `sha1(model+首条用户消息)` 确定性派生 → 同对话稳定、不同对话不同，且能在 opencode 控制台「会话」列认出是哪段；可配 `labelStyle: ascii` → `dsh-Kx7Q`）；**非 ASCII 标签经 `toHeaderValue()` 按 UTF-8 → latin-1 逐字节映射**（undici `Headers.set` 只收 ByteString，见坑 76；线上字节仍是 UTF-8）；已带该头即不覆盖（上游修好自动 no-op）；只克隆读请求体、失败原样放行；卸载即还原。**零运行时依赖、零 peer**（只用 `node:crypto`），产物 `lib/` 入库（`--install-plugin` 不跑构建） | 插件本体（宿主侧，随包分发；**主包不再内置副本**） | ✅ 已发布并**本机已安装**（2026-09-16，`v0.1.1` 覆盖 `v0.1.0`）：`--install-plugin github:lansi-ai/dsh-llm-opencode-session@v0.1.1` 落 `$DSH_HOME/profiles/node_modules` + 装载行 `id: llm-opencode-session`；入口契约实测 `name/inject/apply` 齐备；包内测试 **12 项**全绿；socket 级字节复核（头值 hex = UTF-8）通过；**待实机点验**（发一条消息看是否还有 400，并看「会话」列显示的标签） |
